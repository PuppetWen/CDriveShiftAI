...
每次阶段变化都会先写入项目数据根目录中的 JSON 事务记录，再通知界面。配置与记录写入串行执行，临时文件同步落盘并重命名后才发布新的内存状态。损坏或不可读的已有记录不会被默认配置覆盖，仍可恢复的历史不会因数量上限被裁掉。

切换阶段使用源目录同级备份名，确保原路径所在卷上的重命名是原子的。复制会逐文件核对 SHA-256、目录结构和链接目标，并保留 NTFS 访问权限。符号链接建立、验证并持久化 `linked` 状态后才删除旧副本；清理失败不再切回可能已被部分删除的旧副本。

迁回先持久化 `restorePath` 和 `rolling-back` 状态，校验原路径仍指向记录中的目标后执行切换。原目录恢复后先提交 `rolled-back`，再清理迁移盘副本。启动恢复区分正向迁移和迁回，存在冲突时保留数据供检查。启动恢复结束后才开放 IPC；退出等待文件事务和状态写入完成。

## 强制删除

预检结合进程路径与 Windows Restart Manager 文件占用查询，扫描最多 5000 项且不跟随链接。确认绑定目标文件身份和进程启动时间。执行先尝试直接删除，仅失败后才结束预检中已确认且身份未变的进程；不结束未确认的子进程。删除与迁移互斥，链接外部目标、有效迁移路径、应用自身和系统目录受到保护。

普通尝试使用不提权的 PowerShell 句柄删除，常规重试仍失败时可通过 UAC 启动管理员辅助进程。两种模式均使用固定编码脚本，绑定原预检的卷号、文件 ID、创建时间和规范路径，固定目标及父目录句柄；不递归改写 ACL 或清除外部硬链接属性。辅助进程结束后复验文件确实不存在，主进程退出等待正在执行的删除。

设置可启用或移除当前用户资源管理器的文件/文件夹静态菜单。注册表和设置保存一起回滚失败，启用后从 `--force-delete-path` 接收目标，只加入等待确认的队列。冷启动、驻留和连续请求由同一主进程队列处理，前端共用全局删除弹窗；弹窗显示期间屏蔽后台搜索快捷键。

目录句柄、驱动锁、受保护服务、拒绝访问 ACL、未扫描文件和新启动的写入进程仍可能阻止删除。界面保留具体失败路径、错误码和扫描限制，管理员身份不被视为能够解除所有占用。
