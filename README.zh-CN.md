...
- 像素湖境、未来中枢、月白晶境、熔芯蜂巢、暖瓷晨光五套主题统一加入透亮的液态玻璃材质、背景模糊和边缘高光。
- 路径框、搜索框、工具栏、侧栏、菜单、属性弹窗以及独立搜索、强制删除、卸载前恢复窗口同步适配；卸载前恢复窗口实时跟随主题变化。
- Windows 11 22H2 及以上支持原生 Acrylic；不支持的系统、高对比度或减少透明度模式、材质调用失败时回退到可读的实色背景。
- 指针移动时产生局部反光，不新增闲置循环动画，并尊重减少动画等系统辅助功能偏好。
- 调整文字、输入提示、焦点和危险按钮的辨识度；修复迁移记录数量标签椭圆外多出的矩形底色。
